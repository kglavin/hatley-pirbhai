...
2. **Read inputs.** `intermediate/scan.json` (for project meta + framework hints) and `intermediate/boundary-candidates.json` (per-file kind + routes/topics + evidence).
3. **Group candidates by external actor.** Multiple HTTP routes that all serve "users" → one terminator. A gRPC server endpoint and an HTTP endpoint that both implement the same external API → still one terminator with two flows.
4. **For each group, propose a terminator + its flows.** Emit nodes + edges as above. Be conservative on naming — generic but accurate (`Browser User` not `Chrome Desktop User`) so the architect can rename without renaming HP semantics.
5. **Skip internal boundaries.** Health-check endpoints, debug endpoints, intra-cluster service mesh interfaces. Mark them with a `notes` field saying why you skipped, so the architect can audit.
6. **Set confidence + provenance** on every emitted node/edge.
7. **Write `intermediate/boundary.json`.**
