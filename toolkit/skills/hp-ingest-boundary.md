...
1. **Read the project glossary (H.4.c).** Load `intermediate/glossary.curated.json` (if present — the curator pass is optional). When naming terminators + boundary flows, prefer terms from this glossary over generic English. If a glossary term matches the entity you're naming, use it ("svc-c Producer" not "telemetry sender"; "svc-e Consumer" not "GraphQL client"). Flow labels likewise: project vocabulary > wire-protocol vocabulary.
