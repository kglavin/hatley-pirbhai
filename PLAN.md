...
- *`hp-init` skill wired to code.* `toolkit/scripts/hp_init.py` implements `init_project(name, label, description, dest_dir)` — generates a schema-valid `dictionary.yaml` skeleton (just `sys_root` + commented templates) + a Stage 1 proposal stub (7 standard decisions) + a project README. CLI invocation: `uv run python scripts/hp_init.py <project-name> [--label] [--description]`. Refuses to overwrite existing project dirs. **Verified end-to-end on a fresh `examples/doorbell/` project:** scaffolded → validates clean → renders a (minimal) Context Diagram via `render_project.py`. The full `hp-init → validate → render_project` chain works on a brand-new project with zero prior work.
- *Consolidation pass — documentation refresh.* Top-level `README.md` created (was missing); `toolkit/README.md` rewritten to reflect actual state instead of "(planned)" placeholders; `toolkit/skills/README.md` updated with current skill catalog; `examples/solar/README.md` written to match fishing-rig's README pattern; two new skill files drafted: `hp-init.md` (scaffold a new HP project — directory + dictionary template + Stage 1 proposal) and `hp-status.md` (report what stages a project has reached, validation status, artifact freshness, open questions). Both new skills include sketched-Python implementations as TODOs — the skill *description* is canonical; the code wiring follows once the patterns are stable.
- *🎉 Render pipeline complete — CSPEC generalized; both projects fully transferable.* Added `render_state_machine_elements()` + `wrap_state_machine_html()` to `render/cytoscape.py` (Cytoscape compound nodes for composite states; cross-mode transitions visually distinguished; per-composite styling for known patterns; sensible fallbacks). `scripts/render_project.py` now renders CSPECs automatically for every process with `needs_cspec=True`. **The toolkit is end-to-end transferable:** one script (`render_project.py <dir>`) renders the complete Context + DFD + CSPEC pipeline for any conformant project. Both fishing-rig and solar generate 15 artifacts each (5 per artifact kind), through identical code, with the navigation chain (`context.html` → `dfd.html` → `cspec.html`) working in both. **This closes the "from dictionary.yaml to working diagrams" loop for arbitrary HP projects.**
- *Stage 3 fishing-rig: Bite Detector CSPEC locked.* 9 states + 18 transitions added to fishing-rig dictionary. Flat structure (no composites) — sequence is linear: `Initializing → Idle → Tightening → Armed → BiteDetected → SettingHook → Reeling → Landed → Idle`, plus false-bite branches and Fault paths. Single Fault state per Decision 2 (fishing-rig fault types are smaller/simpler than solar's). Validates clean: 21 entities, 12 flows, 18 transitions, 100% description coverage.
