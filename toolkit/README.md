...
- **Stage 0 (Python; no LLM):** scan files, classify each with an HP **role hint** (`boundary` / `pure-logic` / `state-machine` / `data-store` / `infra` / `config`). The single most decision-shaping signal in the whole pipeline. Deterministic, cheap, runs in seconds even on acme-cp-scale repos.
