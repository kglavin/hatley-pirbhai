...
| `toolkit/hp_toolkit/` | Python package — `model.py` (Pydantic schemas) + `load.py` + `validate.py` (4 validators + CLI) + `render/{mermaid,d2}.py` (level-0 Context Diagram generation). Cytoscape, level-N≥1 DFD, state machines, SVG orchestration planned. | Model + load + validate + level-0 render live |
