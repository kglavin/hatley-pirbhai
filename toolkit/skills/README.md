...
| `hp-capture-adr` | planned | Cross-cutting — mid-decision ADR capture (Modernization #10) |
| `hp-propose-budgets-and-tpms` | planned | Post-Stage-5: NASA design-time budgets + tracked-over-time TPMs (Modernization #21 + #22) |
| `hp-propose-observability` | planned | Per leaf process / module — runtime metrics + alerts + runbooks (Modernization #1 + #33) |
| `hp-propose-slos` | planned | After observability — SLI/SLO/SLA commitments (Modernization #32) |
| `hp-propose-threat-model` | planned | Per cross-trust-zone interconnect — STRIDE + MITRE catalog refs (Modernization #8.2 + #8.3) |
| `hp-propose-bounded-contexts` | planned | When team/vocabulary boundaries become visible — DDD contexts + ACLs (Modernization #5) |
