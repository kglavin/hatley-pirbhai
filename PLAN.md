...
| `toolkit/hp_toolkit/` | Python package — `model.py` (Pydantic schemas: Project, Entity, Flow, Edge, Transition + `is_initial` on Entity) + `load.py` + `validate.py` (4 validators + transition reference checks) + `render/{mermaid,d2}.py` (level-0 Context, level-1 DFD, CSPEC state machine generation). Cytoscape and SVG orchestration planned. | Model + load + validate + Context + DFD + CSPEC render live |
