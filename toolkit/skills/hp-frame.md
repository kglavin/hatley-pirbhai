...
10. **Close when the boundary triage is stable.** When the user can answer "what do you control / influence / not control?" with confidence and the outcomes are operationally precise (effect / standard / conditions), the structured grill is complete. Resist over-framing — Stage 1 will surface things this skill can't.
11. **Close-out re-read prompt.** Before final emit, surface the current `concept.md` to the user and ask: *"Read this cold. What's missing? What feels wrong? What operational detail, day-in-the-life behavior, or edge case did I not ask about?"* The structured grill is necessarily lossy — it covers what the LLM thought to ask. Cold re-read by the user is the safety net for what the LLM didn't think to ask. Capture anything that surfaces; loop back to the relevant branch if needed before declaring done.
12. **Emit `concept.md`.** Frontmatter follows [`hp-frame-concept-format.md`](hp-frame-concept-format.md). Body is the conversation narrative — prose elaborations of each frontmatter field, in the order they came up.
...
- **Value-structure non-linearity** *(specific probe on the Outcomes branch)*. When an outcome's standard is denominated in a measurable unit (kWh, requests, minutes, dollars, units shipped), the *value* of meeting that standard is often non-linear in that unit. Pure CBEA catches the *technical* metric; value-structure probing catches what makes meeting that metric *matter*. Common non-linear cases worth surfacing: time-of-use pricing / rate tiers / time-windowed value, business-hours-vs-off-hours weighting, seasonal pricing, external incentives (rebates / tax credits), regulatory tiers (compliance vs over-compliance), demand charges / peak-shaving value, SLA tiers (p50 vs p99 are different problems). Capture in the outcome's `conditions:` field, or promote to a separate outcome when the non-linearity is significant enough to be its own forcing function.
...
| **Outcomes** | "Who experiences what change? Under what conditions? How fast / accurate / cheap to be 'working'? **Is the value of meeting that standard linear in the unit you're measuring, or non-linear (TOU pricing / rate tiers / time-windowed value / incentives / SLA percentiles)?**" *(If the user names 2+ outcomes, ask for a ranking before going deep on any one.)* | `outcomes[]` (each as `effect / standard / conditions`); non-linear value structure captured in `conditions:` or promoted to a separate outcome |
